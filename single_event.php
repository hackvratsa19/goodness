  ...
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="css/single_event.css">
  </head>
      ...
      <div class="row">
        ...
        <div class="text-center">
                  ...
                  $members_result = mysqli_query($conn, $sql_members);
                      if(mysqli_num_rows($members_result) > 0){ 
                        while($row = mysqli_fetch_assoc($members_result)){ ?>
                          <div class="text-center">
          <img  src="images/<?= $row['picture'] ?>" class="pic" alt="avatar1">
          <span><h4><?= $row['name'] ?></h4></span>
                    ...
                    $sql_members = "SELECT * FROM event_members WHERE user_id =" . $member;
                    $mem_result = mysqli_query($conn, $sql_members);
                    if(mysqli_num_rows($mem_result) > 0){ 
                    } else { ?>
                      <div class="text-center">
                <a href="members_query.php?user=<?php echo $_SESSION['user_id'] ?>&event_id=<?php echo $_GET['event_id'] ?>" class="button"><button class="btn btn-primary">Участвай</button></a>
              </div>
              <?php
                    }
          ...
          <span><h4><?= $row['name'] ?></h4></span>
                ...
                if(mysqli_num_rows($mem_result) > 0){ 
                      ...
                      <div class="text-center">
                <a href="helpers_query.php?user=<?php echo $_SESSION['user_id'] ?>&event_id=<?php echo $_GET['event_id'] ?>" class="button"><button class="btn btn-primary">pom</button></a>
              </div>
              <?php
                    }
